// Generated by DDC, the Dart Development Compiler (to JavaScript).
// Version: 3.3.1 (stable) (Wed Mar 6 13:09:19 2024 +0000) on "macos_arm64"
// Module: packages/plugin_platform_interface/plugin_platform_interface.dart
// Flags: newRuntimeTypes(true), soundNullSafety(true), enableAsserts(true)
define(['dart_sdk'], (function load__packages__plugin_platform_interface__plugin_platform_interface_dart(dart_sdk) {
  'use strict';
  const dart_rti = dart_sdk.dart_rti;
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  var plugin_platform_interface = Object.create(dart.library);
  dart._checkModuleNullSafetyMode(true);
  dart._checkModuleRuntimeTypes(true);
  var T = {
    MockPlatformInterfaceMixin: () => (T.MockPlatformInterfaceMixin = dart.constFn(dart_rti.findType("plugin_platform_interface__plugin_platform_interface|MockPlatformInterfaceMixin")))(),
    VoidTobool: () => (T.VoidTobool = dart.constFn(dart_rti.findType("core|bool()")))(),
    PlatformInterfaceAndObjectTovoid: () => (T.PlatformInterfaceAndObjectTovoid = dart.constFn(dart_rti.findType("~(plugin_platform_interface__plugin_platform_interface|PlatformInterface,core|Object)")))(),
    PlatformInterfaceAndObject__Tovoid: () => (T.PlatformInterfaceAndObject__Tovoid = dart.constFn(dart_rti.findType("~(plugin_platform_interface__plugin_platform_interface|PlatformInterface,core|Object{preventConstObject!core|bool})")))(),
    ExpandoOfObject: () => (T.ExpandoOfObject = dart.constFn(dart_rti.findType("core|Expando<core|Object>")))(),
    Object: () => (T.Object = dart.constFn(dart_rti.findType("core|Object")))()
  };
  var G = {
    ExpandoOfObject: () => (G.ExpandoOfObject = dart.constFn(core.Expando$(T.Object())))()
  };
  const CT = Object.create({
    _: () => (C, CT)
  });
  dart.defineLazy(CT, {
    get C0() {
      return C[0] = dart.const(Object.setPrototypeOf({
      }, core.Object.prototype));
    }
  }, false);
  var C = [void 0];
  var I = [
    "file:///Users/haggaibarel/.pub-cache/hosted/pub.dev/plugin_platform_interface-2.1.8/lib/plugin_platform_interface.dart",
    "package:plugin_platform_interface/plugin_platform_interface.dart",
    "dart:core"
  ];
  var _is = dart.privateName(dart_rti, "_is");
  plugin_platform_interface.PlatformInterface = class PlatformInterface extends core.Object {
    static verify(instance, token) {
      plugin_platform_interface.PlatformInterface._verify(instance, token, {preventConstObject: true});
    }
    static verifyToken(instance, token) {
      plugin_platform_interface.PlatformInterface._verify(instance, token, {preventConstObject: false});
    }
    static _verify(instance, token, opts) {
      let preventConstObject = opts && 'preventConstObject' in opts ? opts.preventConstObject : null;
      if (T.MockPlatformInterfaceMixin()[_is](instance)) {
        let assertionsEnabled = false;
        if (!dart.fn(() => {
          assertionsEnabled = true;
          return true;
        }, T.VoidTobool())()) dart.assertFailed(null, I[0], 90, 14, "() {\n        assertionsEnabled = true;\n        return true;\n      }()");
        if (!assertionsEnabled) {
          dart.throw(new core.AssertionError.new("`MockPlatformInterfaceMixin` is not intended for use in release builds."));
        }
        return;
      }
      if (preventConstObject && plugin_platform_interface.PlatformInterface._instanceTokens._get(instance) === (C[0] || CT.C0)) {
        dart.throw(new core.AssertionError.new("`const Object()` cannot be used as the token."));
      }
      if (token !== plugin_platform_interface.PlatformInterface._instanceTokens._get(instance)) {
        dart.throw(new core.AssertionError.new("Platform interfaces must not be implemented with `implements`"));
      }
    }
  };
  (plugin_platform_interface.PlatformInterface.new = function(opts) {
    let token = opts && 'token' in opts ? opts.token : null;
    plugin_platform_interface.PlatformInterface._instanceTokens._set(this, token);
  }).prototype = plugin_platform_interface.PlatformInterface.prototype;
  dart.addTypeTests(plugin_platform_interface.PlatformInterface);
  dart.addRtiResources(plugin_platform_interface.PlatformInterface, ["plugin_platform_interface__plugin_platform_interface|PlatformInterface"]);
  dart.addTypeCaches(plugin_platform_interface.PlatformInterface);
  dart.setStaticMethodSignature(plugin_platform_interface.PlatformInterface, () => ['verify', 'verifyToken', '_verify']);
  dart.setLibraryUri(plugin_platform_interface.PlatformInterface, I[1]);
  dart.setStaticFieldSignature(plugin_platform_interface.PlatformInterface, () => ['_instanceTokens']);
  dart.defineLazy(plugin_platform_interface.PlatformInterface, {
    /*plugin_platform_interface.PlatformInterface._instanceTokens*/get _instanceTokens() {
      return new (G.ExpandoOfObject()).new();
    }
  }, false);
  plugin_platform_interface.MockPlatformInterfaceMixin = class MockPlatformInterfaceMixin extends core.Object {};
  (plugin_platform_interface.MockPlatformInterfaceMixin.new = function() {
    ;
  }).prototype = plugin_platform_interface.MockPlatformInterfaceMixin.prototype;
  dart.addTypeTests(plugin_platform_interface.MockPlatformInterfaceMixin);
  dart.addRtiResources(plugin_platform_interface.MockPlatformInterfaceMixin, ["plugin_platform_interface__plugin_platform_interface|MockPlatformInterfaceMixin", "plugin_platform_interface__plugin_platform_interface|PlatformInterface"]);
  dart.addTypeCaches(plugin_platform_interface.MockPlatformInterfaceMixin);
  dart.setLibraryUri(plugin_platform_interface.MockPlatformInterfaceMixin, I[1]);
  dart.moduleConstCaches.set("packages/plugin_platform_interface/plugin_platform_interface.dart", C);
  dart_rti._Universe.addRules(dart.typeUniverse, JSON.parse('{"plugin_platform_interface__plugin_platform_interface|MockPlatformInterfaceMixin":{"plugin_platform_interface__plugin_platform_interface|PlatformInterface":[]},"core|Expando":{"Expando.T":"1"}}'));
  dart.trackLibraries("packages/plugin_platform_interface/plugin_platform_interface.dart", {
    "package:plugin_platform_interface/plugin_platform_interface.dart": plugin_platform_interface
  }, {
  }, '{"version":3,"sourceRoot":"","sources":["plugin_platform_interface.dart"],"names":[],"mappings":";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;kBAsEuC,UAAiB;AACpD,0DAAQ,QAAQ,EAAE,KAAK,uBAAsB;IAC/C;uBAM0C,UAAiB;AACzD,0DAAQ,QAAQ,EAAE,KAAK,uBAAsB;IAC/C;mBAGoB,UACX;UACO;AAEd,UAAa,oCAAT,QAAQ;AACL,gCAAoB;AACzB,aAAO,AAGN;AAFC,8BAAoB;AACpB,gBAAO;;AAET,aAAK,iBAAiB;AACpB,qBAAM,4BACF;;AAEN;;AAEF,UAAI,kBAAkB,IAClB,AAAU,AAAe,iEAAC,QAAQ;AACpC,mBAAM,4BAAe;;AAEvB,UAAe,KAAK,KAAE,AAAe,iEAAC,QAAQ;AAC5C,mBAAM,4BACF;;IAER;;;QA/DmC;AAClB,IAAf,iEAAgB,MAAQ,KAAK;EAC/B;;;;;;;;MAW6B,2DAAe;YAAG;;;;;;EAqE6B","file":"../../../../../../../packages/plugin_platform_interface/plugin_platform_interface.dart.lib.js"}');
  // Exports:
  return {
    plugin_platform_interface: plugin_platform_interface
  };
}));

//# sourceMappingURL=plugin_platform_interface.dart.lib.js.map
